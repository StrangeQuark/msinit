pipeline {
    ...
    environment {
        ...
        CICD_TOKEN = credentials('EMAIL_CICD_TOKEN')
    ...
    stages {
                ...
                script {
                    if(VAULTSERVICE_ENABLED == "true") {
                        def response = httpRequest(
                            ...
                            requestBody: '{"serviceName":"emailservice","environmentName":"e3"}',
                            customHeaders: [
                        ...
                        writeFile file: 'emailservice.env', text: response.content
        ...
        stage("Deploy & Health Check") {
            steps {
                script {
                    try {
                        sh "docker compose --env-file emailservice.env up --build -d"
                        ...
                        for (int i = 0; i < maxRetries; i++) {
                            try {
                                ...
                                def healthResponse = httpRequest(
                                    url: 'http://localhost:6005/api/email/health',
                                    validResponseCodes: '200'
                                )
