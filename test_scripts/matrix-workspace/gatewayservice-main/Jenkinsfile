pipeline {
    agent { label 'linux-agent' }
        ...
        environment {
            ...
            CICD_TOKEN = credentials('GATEWAY_CICD_TOKEN')
    ...
    stages {
                ...
                script {
                    if(VAULTSERVICE_ENABLED == "true") {
                        def response = httpRequest(
                            ...
                            requestBody: '{"serviceName":"gatewayservice","environmentName":"e3"}',
                            customHeaders: [
                        ...
                        writeFile file: 'gatewayservice.env', text: response.content
        ...
        stage("Deploy & Health Check") {
            steps {
                script {
                    try {
                        sh "docker compose --env-file gatewayservice.env up --build -d"
